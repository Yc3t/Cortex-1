...
def process_gpt(file_prompt,system_msg):
    model = genai.GenerativeModel(model_name="gemini-1.5-flash",system_instruction= system_msg,generation_config=generation_config)
        ...
        try:
            with open(file, "r") as f:
                ...
                if prompt_template is None:
                    prompt_template = (
                        ""  # Assign a default value if prompt_template is None
                    )
                ...
                if result:  # Check if result is not None or empty
                    try:
                        ...
                        print(results.append(json.loads(result)))
                    except json.JSONDecodeError as json_err:
                        print(f"JSON decoding failed for {file}: {json_err}")
                else:
                    print(f"Result is None or empty for {file}")
